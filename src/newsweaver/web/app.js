...
function renderPreview() {
  ...
  $("articleCount").textContent = `${data.articles.length} 篇`; list.className = "article-list"; list.innerHTML = data.articles.map((a,i) => `<article class="article-card"><div class="article-index">${String(i+1).padStart(2,"0")}</div><div><h4>${escapeHtml(a.title)}</h4><div class="article-meta"><span>${escapeHtml(a.source || "未知")}</span><span>${formatDate(a.published_at)}</span><span>综合分 ${(a.ranking?.final_score || 0).toFixed(3)}</span><span>事件 ${escapeHtml(a.ranking?.event_id || "未聚类")}</span><span>${a.extraction?.status === "success" ? `✓ 正文 ${a.extraction.content_length || 0} 字` : "仅摘要"}</span></div><a href="${safeUrl(a.url)}" target="_blank" rel="noreferrer">查看原文</a></div></article>`).join("");
}
...
async function pollJob(id) {
  while (true) {
    ...
    if (job.status === "complete") { applyState(await api("/api/state"), await api("/api/health")); $("reportSelect").value = job.report; await loadReport(); activateTab("report"); setStatus(job.message || "报告已生成", job.audit_status === "needs_review" ? "warn" : "ok"); setTimeout(hideProgress, 1000); return; }
...
function renderEvidence(facts) { $("evidenceCount").textContent = `${facts.length} 条`; $("evidenceList").className = facts.length ? "evidence-list" : "evidence-list empty-state"; $("evidenceList").innerHTML = facts.length ? facts.map(f => { const id = f.fact_id || f.id; return `<article class="evidence-card" id="fact-${escapeHtml(id)}"><strong>${escapeHtml(id)}</strong><p>${escapeHtml(f.claim)}</p><small>${escapeHtml(f.source)} · ${formatDate(f.published_at)} · ${escapeHtml(f.event_id || "未聚类")}</small><details><summary>查看原始证据</summary><p>${escapeHtml(f.source_span || f.claim)}</p></details><a href="${safeUrl(f.url)}" target="_blank" rel="noreferrer">原文 ↗</a></article>`; }).join("") : "暂无证据"; }
function renderAudit(audit) { const warnings = audit.warnings || [], status = audit.status || (audit.valid ? "pass" : "needs_review"), passed = ["pass","repaired"].includes(status); const labels = {pass:"✓ 审计通过", repaired:"✓ 自动修复后通过", needs_review:"⚠ 需要人工复核", failed:"⚠ 审计失败"}; $("auditBox").className = `audit-box ${passed ? "valid" : "warn"}`; $("auditBox").innerHTML = audit.citation_coverage == null ? "暂无引用审计" : `<strong>${labels[status] || labels.needs_review}</strong><span>证据覆盖 ${audit.citation_coverage}% · 已引用 ${(audit.cited_ids||[]).length} 条 · 修复 ${audit.repair_attempts||0} 次</span>${warnings.length ? `<small>${warnings.join("；")}</small>` : ""}`; }
...
async function loadTrend() { const topic = $("topicSelect").value; if (!topic) return; const t = await api(`/api/trend?topic=${encodeURIComponent(topic)}`), change = t.change_since_last_period || {}; $("trendView").className = "trend-grid"; $("trendView").innerHTML = `<article class="trend-hero"><span>趋势判断</span><h3>${escapeHtml(t.trend_conclusion || "暂无足够数据")}</h3><p>L2 ${t.memory_depth?.recent_entries || 0} 条 · L3 ${t.memory_depth?.weekly_trends || 0} 周</p></article>${trendCard("本期热点", t.current_hotspots)}${trendCard("反复玩家", t.recurring_players)}${trendCard("新事实", t.what_is_new)}${trendCard("已确认", t.what_was_confirmed)}${trendCard("被质疑", t.what_was_contradicted)}${trendCard("预测兑现", t.what_was_fulfilled)}${trendCard("已过时", t.what_became_outdated)}${trendCard("新出现玩家", change.new_players)}${trendCard("降温玩家", change.fading_players)}${trendCard("拐点信号", t.turning_points)}`; }
